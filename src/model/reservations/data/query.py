...
class RelBuilder(QueryBuilder):
    ...
    def get(self, id: Optional[str], user: Optional[str], venue: Optional[str], time: Optional[Tuple[str, str]], people: Optional[Tuple[int, int]]) -> List[ReservationSchema]:
...
class MockedBuilder(QueryBuilder):
    ...
    def __filter(self, values: List[ReservationSchema], by: Callable[[ReservationSchema], bool]) -> List[ReservationSchema]:
        result = []
        for value in values:
            if by(value):
                result.append(value)
    ...
    def __filter_by_eq(self, user: Optional[str], venue: Optional[str]) -> List[ReservationSchema]:
        ...
        if user:
            result = self.__filter(result, self.__filter_by_user(user))
        if venue:
            result = self.__filter(result, self.__filter_by_venue(venue))
