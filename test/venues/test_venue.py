...
def test_new_venue_is_open():
    venue = create_venue("La Pizzeria","125 Main St", 50, logo="foto.url", pictures = ["foto1", "foto2"], slots=[datetime.now(),datetime.now()], characteristics= ["Arepas", "Cafeteria"], vacations=[datetime.now()], reservationLeadTime=10,menu="comidas.url")
    assert venue.get_status() == Available().get_status()

def test_closing_a_venue_changes_status_to_closed():
    venue = create_venue(name="La Pizzeria", location="123 Main St", capacity=50, logo="foto.url", pictures = ["foto1", "foto2"], slots=[datetime.now()], characteristics= ["Arepas", "Cafeteria"], vacations=[datetime.now()], reservationLeadTime=10,menu="comidas.url")
    venue.close()
...
def test_updating_a_venue_changes_its_attributes():
    venue = create_venue(name="La Pizzeria", location="123 Main St", capacity=50, logo="foto.url", pictures = ["foto1", "foto2"], slots=[datetime.now()], characteristics= ["Arepas", "Cafeteria"], vacations=[datetime.now()], reservationLeadTime=10,menu="comidas.url")
    update = Update(name="La Trattoria", location="456 Main St", capacity=60, logo="nuevafoto.url", pictures = ["foto3", "foto4"], slots=[datetime.now(),datetime.now()], characteristics= ["Carnes", "Comida Armenia"], vacations=[datetime.now(),datetime.now()], reservationLeadTime=20,menu="comidas2.url")
...
def test_the_venue_can_close_through_an_update():
    venue = create_venue("La Pizzeria", "123 Main St", 50, logo="foto.url", pictures = ["foto1", "foto2"], slots=[datetime.now()], characteristics= ["Arepas", "Cafeteria"], vacations=[datetime.now()], reservationLeadTime=10,menu="comidas.url")
    venue.open()
...
def create_venues_by_distance() -> List[Venue]:
    ...
    venues = [create_venue("La Pizzerias", 
                           ...
                           slots=[datetime.now()], characteristics= ["Arepas", "Cafeteria"], 
