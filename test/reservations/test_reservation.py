...
def test_after_persisting_a_reservation_it_can_be_recovered_with_its_id():
    ...
    assert base.get_reservation_by_id(reservation.id).id == reservation.id
...
def test_a_reservation_is_done_if_the_venue_does_exist():
    ...
    venues = LocalVenuesProvider(venues_db)
    service = LocalReservationsProvider(database, venues)
    asyncio.run(venues.create_venue(v.CreateInfo(name="Lo de Carlitos", location="almendra 270", capacity=150)))
